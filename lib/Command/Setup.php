...
class Setup extends Command
{
    ...
    protected function configure(): void
    {
        $this
            ...
            ->addOption('auth', null, InputOption::VALUE_REQUIRED, 'Auth type: oauth (default/SSO) or plain (legacy)', 'oauth')
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        if (!$skipChecks) {
            ...
            if ($imapResult['connected']) {
                ...
                if ($isOAuth && !$hasOAuth) {
                    $output->writeln('  <error>✗ IMAP server does not support OAUTHBEARER/XOAUTH2</error>');
                    $output->writeln('    Dovecot: https://doc.dovecot.org/configuration_manual/authentication/oauth2/');
                    $errors++;
                }
            ...
            if ($smtpResult['connected']) {
                $banner = $smtpResult['banner'] ? \preg_replace('/^220\s*/', '', $smtpResult['banner']) : '';
                $output->writeln("  <info>✓ SMTP  {$smtpHost}:{$smtpPort}" . ($banner ? " ({$banner})" : '') . '</info>');
            } else {
        ...
        if (\is_dir($appDir)) {
            try {
                ...
                $appPath = \preg_replace('#(?<!:)/+#', '/', \rtrim($this->appManager->getAppWebPath(self::APP_ID), '/') . '/app/');
