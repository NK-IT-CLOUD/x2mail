...
class Status extends Command
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $output->writeln('  Provider:        ' . ($provider !== 'none' ? "<info>{$provider}</info>" : '<error>none installed</error>'));

        if ($userOidc) {
            $storeToken = $this->appConfig->getValueString('user_oidc', 'store_login_token', '0');
            $output->writeln('  Token store:     ' . ($storeToken === '1' ? '<info>enabled</info>' : '<error>disabled (set store_login_token=1)</error>'));

            try {
                $hasProvider = $this->appConfig->getValueString('user_oidc', 'provider-1-mappingUid', '', lazy: true) !== '';
                $output->writeln('  OIDC provider:   ' . ($hasProvider ? '<info>configured</info>' : '<comment>not detected (occ user_oidc:provider)</comment>'));
            } catch (\Throwable $e) {
                // Cannot check — skip
            }
        }
        ...
        $output->writeln('');
        $output->writeln('  <comment>Token diagnostics only available in browser wizard (Admin → X2Mail → Run Checks)</comment>');
        ...
        if (\is_dir($appDir)) {
            try {
                \OCA\X2Mail\Util\EngineHelper::loadApp();
                $output->writeln('  Version: ' . APP_VERSION);
                $output->writeln('  app_path: ' . \X2Mail\Engine\Api::Config()->Get('webmail', 'app_path', '(not set)'));
            } catch (\Throwable $e) {
