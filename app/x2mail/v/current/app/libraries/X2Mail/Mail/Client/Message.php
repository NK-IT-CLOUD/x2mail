...
//		$aFlags = [],
	...
	private ?\X2Mail\Mail\Mime\EmailCollection
		$oFrom = null,
		$oSender = null,
		$oReplyTo = null,
		$oDeliveredTo = null,
...
//		$oMessage->sEmailId = $oMessage->sEmailId ?: $oFetchResponse->GetFetchValue('X-GUID');
		...
		if ($oHeaders) {
			...
			$sContentTypeCharset = $oHeaders->ParameterValue(
				...
				\X2Mail\Mail\Mime\Enumerations\Parameter::CHARSET
			);
...
//			$oMessage->sDeliveryReceipt = \trim($oHeaders->ValueByName(MimeHeader::RETURN_RECEIPT_TO));
		...
		{
			$sCharset = $sCharset ?: \X2Mail\Mail\Base\Enumerations\Charset::ISO_8859_1;
...
*/
			...
			if ($aTextParts) {
				$sCharset = $sCharset ?: \X2Mail\Mail\Base\Enumerations\Charset::UTF_8;
				...
				foreach ($aTextParts as $oPart) {
					...
					if (\is_string($sText) && \strlen($sText)) {
						...
						$sText = Utils::ConvertEncoding($sText,
							...
							\X2Mail\Mail\Base\Enumerations\Charset::UTF_8
						);
