...
class BodyStructure implements \JsonSerializable
{
	...
	/**
	 * \X2Mail\Mail\Imap\BodyStructure[]
	 */
...
//		return 'application/pgp-encrypted' !== $this->sContentType
	...
	public function GetHtmlAndPlainParts() : array
	{
		 ...
		 * Only when \X2Mail\Engine\Api::Config()->Get('security', 'openpgp', true)
		 */
		if (!$aParts) {
	...
	public function SearchByCallback(callable $fCallback, /*BodyStructure*/ $parent = null) : iterable
	{
		...
		foreach ($this->aSubParts as /* @var $oSubPart \X2Mail\Mail\Imap\BodyStructure */ $oSubPart) {
...
//			return $oItem->IsAttachment();
	...
	private static function decodeAttrParameter(array $aParams, string $sParamName, string $sCharset) : string
	{
		...
		if (isset($aParams[$sParamName])) {
			$sResult = \X2Mail\Mail\Base\Utils::DecodeHeaderValue($aParams[$sParamName], $sCharset);
		} else if (isset($aParams[$sParamName.'*'])) {
			$aValueParts = \explode("''", $aParams[$sParamName.'*'], 2);
			if (2 === \count($aValueParts)) {
				$sCharset = isset($aValueParts[0]) ? $aValueParts[0] : \X2Mail\Mail\Base\Enumerations\Charset::UTF_8;
				$sResult = \X2Mail\Mail\Base\Utils::ConvertEncoding(
					\urldecode($aValueParts[1]), $sCharset, \X2Mail\Mail\Base\Enumerations\Charset::UTF_8);
			} else {
			...
			if (\count($aFileNames)) {
				...
				if (\strlen($sCharset)) {
					$sResult = \X2Mail\Mail\Base\Utils::ConvertEncoding($sResult,
						$sCharset, \X2Mail\Mail\Base\Enumerations\Charset::UTF_8);
				}
			}
		}
	...
	public static function NewInstance(array $aBodyStructure, string $sPartID = '') : ?self
	{
		...
		$oStructure->sFileName = \X2Mail\Mail\Base\Utils::Utf8Clear($sFileName);
...
//			'mimeTypeParams' => $this->aContentTypeParams,
			'fileName' => \X2Mail\Mail\Base\Utils::SecureFileName($this->FileName(true)),
