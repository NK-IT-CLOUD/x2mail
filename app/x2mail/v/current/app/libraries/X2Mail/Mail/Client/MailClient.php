...
//			MimeHeader::RETURN_RECEIPT_TO,
	 ...
	 * @throws \X2Mail\Mail\Imap\Exceptions\*
	 * @throws \X2Mail\Mail\Client\Exceptions\*
	 */
...
//			$aInfo = $this->oImapClient->FolderExamine($sFolderName)->jsonSerialize();
			...
			if (\count($oRange)) {
				...
				foreach ($aFetchResponse as $oFetchResponse) {
					$iUid = (int) $oFetchResponse->GetFetchValue(FetchType::UID);
					$aLowerFlags = \array_map('mb_strtolower', \array_map('\\X2Mail\Mail\\Base\\Utils::Utf7ModifiedToUtf8', $oFetchResponse->GetFetchValue(FetchType::FLAGS)));
					$aInfo['messagesFlags'][] = array(
						'uid' => $iUid,
						'flags' => $aLowerFlags
					);
