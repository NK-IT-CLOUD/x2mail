...
class Manager
{
	...
	private \X2Mail\Engine\Actions $oActions;
	...
	public function __construct(\X2Mail\Engine\Actions $oActions)
	{
	...
	public function CreatePluginByName(string $sName) : ?\X2Mail\Engine\Plugins\AbstractPlugin
	{
		$oPlugin = null;
		...
		if ($sClassName) {
			...
			$oPlugin
				...
				->SetPluginConfig(new \X2Mail\Engine\Config\Plugin($sName, $oPlugin->ConfigMap(true)))
			;
	...
	public function loadPluginByName(string $sName) : ?string
	{
		if (\preg_match('/^[a-z0-9\\-]+$/', $sName)) {
			...
			if (\class_exists($sClassName) && \is_subclass_of($sClassName, 'X2Mail\\Engine\\Plugins\\AbstractPlugin')) {
				return $sClassName;
			}
	...
	public function Actions() : \X2Mail\Engine\Actions
	{
	...
	public function InitAppData(bool $bAdmin, array &$aAppData, ?\X2Mail\Engine\Model\Account $oAccount = null) : self
	{
		if ($this->bIsEnabled && isset($aAppData['Plugins']) && \is_array($aAppData['Plugins'])) {
			...
			foreach ($this->aPlugins as $oPlugin) {
				if ($oPlugin) {
					...
					if (\is_array($aMap)) {
						foreach ($aMap as /* @var $oPluginProperty \X2Mail\Engine\Plugins\Property */ $oPluginProperty) {
