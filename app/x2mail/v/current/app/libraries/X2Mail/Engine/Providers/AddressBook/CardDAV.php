...
use X2Mail\Engine\DAV\Client as DAVClient;
...
trait CardDAV
{
					...
					if (isset($aItem['{DAV:}getetag'])) {
						 ...
						 && !static::hasDAVCollection($aItem))
						{
							...
							if (isset($aItem['{DAV:}getlastmodified'])) {
								...
								$mResult[$sKeyID]['changed'] = \X2Mail\Mail\Base\DateTimeHelper::ParseRFC2822DateString(
									$aItem['{DAV:}getlastmodified']);
							} else {
								$mResult[$sKeyID]['changed'] = \X2Mail\Mail\Base\DateTimeHelper::TryToParseSpecEtagFormat($mResult[$sKeyID]['etag']);
	...
	protected function davClientRequest(DAVClient $oClient, string $sCmd, string $sUrl, $mData = null) : ?\X2Mail\Engine\HTTP\Response
	{
