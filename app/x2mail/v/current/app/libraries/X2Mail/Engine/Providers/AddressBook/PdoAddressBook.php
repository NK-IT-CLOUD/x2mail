...
use
	...
	X2Mail\Engine\Providers\AddressBook\Classes\Contact,
	X2Mail\Engine\Providers\AddressBook\Enumerations\PropertyType
;

class PdoAddressBook
	extends \X2Mail\Engine\Pdo\Base
	implements AddressBookInterface
{
	...
	private \X2Mail\Engine\Pdo\Settings $settings;
	...
	public function __construct()
	{
		...
		$oSettings = new \X2Mail\Engine\Pdo\Settings;
		...
		if ('sqlite' === $oSettings->driver) {
			...
			if (!$oConfig->Get('contacts', 'sqlite_global', \is_file(APP_PRIVATE_DATA . '/AddressBook.sqlite'))) {
				$oAccount = \X2Mail\Engine\Api::Actions()->getMainAccountFromToken(false);
				if ($oAccount) {
					$homedir = \X2Mail\Engine\Api::Actions()->StorageProvider()->GenerateFilePath(
						$oAccount,
						\X2Mail\Engine\Providers\Storage\Enumerations\StorageType::ROOT
					);
...
//					}
	...
	{
		return !!$this->prepareAndExecute('DELETE FROM x2mail_ab_contacts WHERE id_user = :id_user AND deleted = 1', array(
	...
	{
		if (1 > $this->iUserID) {
			\X2Mail\Engine\Log::warning('PdoAddressBook', 'Sync() invalid $iUserID');
			return false;
		}

		$oClient = $this->getDavClient();
		if (!$oClient) {
			\X2Mail\Engine\Log::warning('PdoAddressBook', 'Sync() invalid DavClient');
			return false;
		}
		...
		if (false === $aRemoteSyncData) {
			\X2Mail\Engine\Log::info('PdoAddressBook', 'Sync() no data to sync');
			return false;
		}
		$time = \microtime(true) - $time;
		\X2Mail\Engine\HTTP\Stream::JSON(['messsage'=>"Received ".\count($aRemoteSyncData)." remote contacts in {$time} seconds"]);
		\X2Mail\Engine\Log::info('PdoAddressBook', \count($aRemoteSyncData) . ' remote contacts');

		$aLocalSyncData = $this->prepareDatabaseSyncData();
		\X2Mail\Engine\Log::info('PdoAddressBook', \count($aLocalSyncData) . ' local contacts');
...
//		$this->oLogger->WriteDump($aLocalSyncData);
		...
		if ($bReadWrite) {
			\X2Mail\Engine\Log::info('PdoAddressBook', 'Sync() is import and export');
			$iCount = 0;
			foreach ($aLocalSyncData as $sKey => $aData) {
				if ($aData['deleted']) {
					...
					if (isset($aRemoteSyncData[$sKey], $aRemoteSyncData[$sKey]['vcf'])) {
						\X2Mail\Engine\HTTP\Stream::JSON(['messsage'=>"Delete remote {$sKey}"]);
						$this->davClientRequest($oClient, 'DELETE', $sPath.$aRemoteSyncData[$sKey]['vcf']);
					}
				}
			}
			if ($iCount) {
				\X2Mail\Engine\Log::info('PdoAddressBook', $iCount . ' remote contacts removed');
			}
		} else {
			\X2Mail\Engine\Log::info('PdoAddressBook', 'Sync() is import only');
		}
		...
		if (\count($aIdsForDeletion)) {
			\X2Mail\Engine\HTTP\Stream::JSON(['messsage'=>'Delete local ' . \implode(', ', $aIdsForDeletion)]);
			$this->DeleteContacts($aIdsForDeletion);
			\X2Mail\Engine\Log::info('PdoAddressBook', \count($aIdsForDeletion) . ' local contacts removed');
			...
			foreach ($aLocalSyncData as $sKey => $aData) {
				if ((empty($aData['etag']) && !isset($aRemoteSyncData[$sKey])) // new
				 ...
				 || (!empty($aData['etag']) && isset($aRemoteSyncData[$sKey]) &&
					...
					\X2Mail\Engine\HTTP\Stream::JSON(['messsage'=>"Update remote {$sKey}"]);
					...
					if ($oContact) {
						...
						\X2Mail\Engine\Log::info('PdoAddressBook', "Update contact {$sKey} in DAV");
						...
						if ($oResponse) {
							...
							if (!empty($sEtag)) {
								$iChanged = empty($sDate) ? \time() : \X2Mail\Mail\Base\DateTimeHelper::ParseRFC2822DateString($sDate);
								$this->prepareAndExecute('UPDATE x2mail_ab_contacts SET changed = :changed, etag = :etag '.
							...
							\X2Mail\Engine\Log::warning('PdoAddressBook', "Update/create remote failed");
						}
					} else {
						\X2Mail\Engine\Log::warning('PdoAddressBook', "Local contact {$sKey} not found");
					}
					unset($oContact);
				}
			}
		...
		foreach ($aRemoteSyncData as $sKey => $aData) {
			if (!isset($aLocalSyncData[$sKey]) // new
			 ...
			 || ($aLocalSyncData[$sKey]['etag'] !== $aData['etag'] && $aLocalSyncData[$sKey]['changed'] < $aData['changed'])
				...
				\X2Mail\Engine\HTTP\Stream::JSON(['messsage'=>"Update local {$sKey}"]);
				...
				if ($oVCard instanceof VCard) {
					...
					if ($oContact) {
						\X2Mail\Engine\Log::info('PdoAddressBook', "Update local contact {$sKey}");
					} else {
						\X2Mail\Engine\Log::info('PdoAddressBook', "Create local contact {$sKey}");
					...
					\X2Mail\Engine\Log::error('PdoAddressBook', "Import remote contact {$sKey} failed");
				}
			}
		}
	...
	{
		if (1 > $this->iUserID) {
			\X2Mail\Engine\Log::warning('PdoAddressBook', 'Export() invalid $iUserID');
			return false;
		}
...
//					if ($mData && isset($mData['id_contact'], $mData['deleted']) && !$mData['deleted']) {
	...
	{
		if (1 > $this->iUserID) {
			\X2Mail\Engine\Log::warning('PdoAddressBook', 'ContactSave() invalid $iUserID');
			return false;
		}
...
//		$oContact->REV = \time();
		...
		try {
			...
			if ($bUpdate) {
				...
				$this->prepareAndExecute('UPDATE x2mail_ab_contacts
				...
				$this->prepareAndExecute(
					'DELETE FROM x2mail_ab_properties WHERE id_user = :id_user AND id_contact = :id_contact',
					array(
						':id_user' => array($this->iUserID, \PDO::PARAM_INT),
						':id_contact' => array($iIdContact, \PDO::PARAM_INT)
					)
				...
				$this->prepareAndExecute('INSERT INTO x2mail_ab_contacts
				...
				$sLast = $this->lastInsertId('x2mail_ab_contacts', 'id_contact');
			...
			if (0 < $iIdContact) {
				...
				if ($aParams) {
					$this->prepareAndExecute('INSERT INTO x2mail_ab_properties '.
	...
	{
		if (1 > $this->iUserID) {
			\X2Mail\Engine\Log::warning('PdoAddressBook', 'DeleteContacts() invalid $iUserID');
			return false;
		}
		...
		$this->prepareAndExecute('DELETE FROM x2mail_ab_properties WHERE id_user = :id_user AND id_contact IN ('.$sIDs.')', $aParams);
		...
		$this->prepareAndExecute('UPDATE x2mail_ab_contacts SET deleted = 1, changed = :changed '.
	...
	public function DeleteAllContacts(string $sEmail) : bool
	{
		...
		$this->prepareAndExecute('DELETE FROM x2mail_ab_properties WHERE id_user = :id_user', $aParams);
		$this->prepareAndExecute('DELETE FROM x2mail_ab_contacts WHERE id_user = :id_user', $aParams);
...
//							$oContact->vCard->REV = \gmdate('Ymd\\THis\\Z', $oContact->Changed);
			...
			// Build vCards using legacy data (missing jCard)
			if (\count($aIdContacts)) {
				$oStmt->closeCursor();

				$oStmt = $this->prepareAndExecute('SELECT * FROM x2mail_ab_properties
						...
						foreach ($aFetch as $aItem) {
							if ($aItem && isset($aItem['id_prop'], $aItem['id_contact'], $aItem['prop_type'], $aItem['prop_value'])) {
								...
								if (0 < $iId) {
									if ($iPrevId != $iId) {
										if ($iPrevId) {
											$aVCards[$iPrevId]->UID = $sUid ?: \X2Mail\Engine\UUID::generate();
...
//										$oVCard = $oVCard->convert(VCard::VCARD40);
										$oVCard->VERSION = '4.0';
										$oVCard->PRODID = 'X2Mail-'.APP_VERSION;
										$aVCards[$iId] = $oVCard;
									}
									$oVCard = $aVCards[$iId];
	...
	public function GetContacts(int $iOffset = 0, int $iLimit = 20, string $sSearch = '', int &$iResultCount = 0) : array
	{
		...
		if (\strlen($sSearch)) {
			...
			$sSql = 'SELECT DISTINCT id_contact FROM x2mail_ab_properties '.
...
(\strlen($sLowerSearch) ? ' OR (prop_value_lower <> \'\' AND prop_value_lower LIKE :search_lower ESCAPE \'=\')' : '').
			...
			$oStmt = $this->prepareAndExecute(
				'SELECT COUNT(*) FROM x2mail_ab_contacts WHERE id_user = :id_user',
				[':id_user' => array($this->iUserID, \PDO::PARAM_INT)]
			);
		...
		if (0 < $iResultCount) {
			...
			FROM x2mail_ab_contacts AS c
			LEFT JOIN x2mail_ab_properties AS p ON (p.id_contact = c.id_contact AND p.prop_type = :prop_type)
	...
	public function GetSuggestions(string $sSearch, int $iLimit = 20) : array
	{
		...
		$sSql = 'SELECT id_contact, id_prop, prop_type, prop_value FROM x2mail_ab_properties '.
...
(\strlen($sLowerSearch) ? ' OR (prop_value_lower <> \'\' AND prop_value_lower LIKE :search_lower ESCAPE \'=\')' : '').
		...
		if ($oStmt) {
			...
			if (\count($aIdContacts)) {
				...
				$sSql = 'SELECT id_prop, id_contact, prop_type, prop_value FROM x2mail_ab_properties '.
	...
	public function IncFrec(array $aEmails, bool $bCreateAuto = true) : bool
	{
		...
		$aEmailsObjects = \array_map(function ($mItem) {
			$oResult = null;
			try {
				$oResult = \X2Mail\Mail\Mime\Email::Parse(\trim($mItem));
			}
			catch (\Throwable $oException) {
				unset($oException);
			}
		...
		if ($bCreateAuto) {
			$sSql = 'SELECT prop_value FROM x2mail_ab_properties WHERE id_user = :id_user AND prop_type = :prop_type';
		...
		$sSql = 'UPDATE x2mail_ab_properties SET prop_frec = prop_frec + 1 WHERE id_user = :id_user AND prop_type = :prop_type';
	...
	public function Test() : string
	{
		...
		try {
			...
			if (!empty($sResult) && !\X2Mail\Mail\Base\Utils::IsAscii($sResult) && !\X2Mail\Mail\Base\Utils::IsUtf8($sResult)) {
	...
	private function getContactFreq(int $iUserID, int $iIdContact) : array
	{
		...
		$sSql = 'SELECT prop_value, prop_frec FROM x2mail_ab_properties WHERE id_user = :id_user AND id_contact = :id_contact AND prop_type = :type';
		$aParams = array(
	...
	protected function getPdoSettings() : \X2Mail\Engine\Pdo\Settings
	{
...
//			$sSslCa = (\is_file($sFile) || \file_put_contents($sFile, $sSslCa)) ? $sFile : '';
	...
	protected function getUserId(string $sEmail, bool $bSkipInsert = false, bool $bCache = true) : int
	{
		...
		$sEmail = \X2Mail\Engine\IDN::emailToAscii(\trim($sEmail));
		if (empty($sEmail)) {
		...
		$oStmt = $this->prepareAndExecute('SELECT id_user FROM x2mail_users WHERE rl_email = :rl_email',
			array(
		...
		if (!$bSkipInsert) {
			$oStmt->closeCursor();
			$oStmt = $this->prepareAndExecute('INSERT INTO x2mail_users (rl_email) VALUES (:rl_email)',
