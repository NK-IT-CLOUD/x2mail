...
//				$oDriver = new \X2Mail\Engine\Providers\AddressBook\PdoAddressBook();
	...
	public function DoContactsSync() : array
	{
		...
		if (!$oAddressBookProvider) {
			throw new \X2Mail\Engine\Exceptions\ClientException(\X2Mail\Engine\Notifications::ContactsSyncError, null, 'No AddressBookProvider');
		}
		\ignore_user_abort(true);
		\X2Mail\Engine\HTTP\Stream::start(/*$binary = false*/);
		\X2Mail\Engine\HTTP\Stream::JSON(['messsage'=>'start']);
		if (!$oAddressBookProvider->Sync()) {
			throw new \X2Mail\Engine\Exceptions\ClientException(\X2Mail\Engine\Notifications::ContactsSyncError, null, 'AddressBookProvider->Sync() failed');
		}
	...
	public function UploadContacts(?array $aFile, int $iError) : array
	{
		...
		if ($oAccount && UPLOAD_ERR_OK === $iError && \is_array($aFile)) {
			...
			if (!$this->FilesProvider()->MoveUploadedFile($oAccount, $sSavedName, $aFile['tmp_name'])) {
				$iError = \X2Mail\Engine\Enumerations\UploadError::ON_SAVING;
			} else {
	...
	public function setContactsSyncData(\X2Mail\Engine\Model\Account $oAccount, array $aData) : bool
	{
		if (!isset($aData['Mode'])) {
...
//		$oAccount = $this->getAccountFromToken();
		...
		if ($aData['Password']) {
			$aData['Password'] = \X2Mail\Engine\Crypt::EncryptToJSON($aData['Password'], $oMainAccount->CryptKey());
		}
	...
	protected function getContactsSyncData(\X2Mail\Engine\Model\Account $oAccount) : ?array
	{
//		$oAccount = $this->getAccountFromToken();
		...
		if (!empty($sData)) {
			...
			if ($aData) {
				if ($aData['Password']) {
					...
					if ($aData['PasswordHMAC'] !== \hash_hmac('sha1', $aData['Password'], $oMainAccount->CryptKey())) {
						...
						// Success
						$aData['Password'] = \X2Mail\Engine\Crypt::DecryptFromJSON(
							$aData['Password'],
			...
			return \X2Mail\Engine\Upgrade::ConvertInsecureContactsSync($this, $oAccount);
		}
		return null;
	}
