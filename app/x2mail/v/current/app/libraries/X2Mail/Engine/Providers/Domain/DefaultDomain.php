...
use X2Mail\Mail\Cache\CacheClient;
...
class DefaultDomain implements DomainInterface
{
	...
	{
//		return ('*' === $sName) ? 'default' : \str_replace('*', '_wildcard_', \X2Mail\Engine\IDN::toAscii($sName));
...
//		$aList = \glob($this->sDomainPath.'/*.{ini,json,alias}', GLOB_BRACE);
	...
	{
//		$sName = \X2Mail\Engine\IDN::toAscii($sName);
		...
		if (\file_exists($sRealFileBase.'.json')) {
			...
			return \X2Mail\Engine\Model\Domain::fromArray($sName, $aDomain);
		}

		if (\file_exists($sRealFileBase.'.ini')) {
			$aDomain = \parse_ini_file($sRealFileBase.'.ini') ?: array();
			return \X2Mail\Engine\Model\Domain::fromIniArray($sName, $aDomain);
		}
		...
		if ($bFindWithWildCard) {
			...
			$sFoundValue = '';
			if (\strlen($sNames)
			 && \X2Mail\Engine\Plugins\Helper::ValidateWildcardValues($sName, $sNames, $sFoundValue)
			 && \strlen($sFoundValue)
			) {
	...
	public function Save(\X2Mail\Engine\Model\Domain $oDomain) : bool
	{
		...
		\X2Mail\Engine\Utils::saveFile("{$this->sDomainPath}/{$sRealFileName}.json", \json_encode($oDomain, \JSON_PRETTY_PRINT));
	...
	{
//		$sTarget = \X2Mail\Engine\IDN::toAscii($sTarget);
...
//		$this->Delete($sName);
		\X2Mail\Engine\Utils::saveFile("{$this->sDomainPath}/{$sRealFileName}.alias", $sTarget);
		$this->oCacher && $this->oCacher->Delete(static::CACHE_KEY);
		return true;
	}
	...
	protected function getDisabled() : array
	{
		...
		$sItem = \strtok($sFile, ",\n");
		while (false !== $sItem) {
//			$aDisabled[] = \X2Mail\Engine\IDN::toAscii($sItem);
...
//		$sName = \X2Mail\Engine\IDN::toAscii($sName);
		...
		if ($sName) {
			...
			\X2Mail\Engine\Utils::saveFile($this->sDomainPath.'/disabled', \implode("\n", $aResult));
