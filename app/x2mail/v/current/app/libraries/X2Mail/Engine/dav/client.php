...
class Client
{
	...
	function __construct(array $settings)
	{
		...
		$this->HTTP = \X2Mail\Engine\HTTP\Request::factory(/*'socket'*/);
		$this->HTTP->proxy = $settings['proxy'] ?? null;
		if (!empty($settings['userName']) && !empty($settings['password'])) {
			$this->HTTP->setAuth(3, $settings['userName'], $settings['password']);
		} else {
			\X2Mail\Engine\Log::warning('DAV', 'No user credentials set');
		}
	 ...
	 */
	public function request(string $method, string $url = '', ?string $body = null, array $headers = array()) : \X2Mail\Engine\HTTP\Response
	{
		...
		\X2Mail\Engine\Log::debug('DAV', "{$method} {$url}" . ($body ? "\n\t" . \str_replace("\n", "\n\t", $body) : ''));
		...
		if (301 == $response->status) {
			...
			\X2Mail\Engine\Log::info('DAV', "301 Redirect {$url} to {$location}");
		...
		if (300 <= $response->status) {
			throw new \X2Mail\Engine\HTTP\Exception("{$method} {$url}", $response->status, $response);
		}
		\X2Mail\Engine\Log::debug('DAV', "{$response->status}: {$response->body}");
