...
trait User
{
	...
	public function DoAppDelayStart() : array
	{
		...
		$sTimers = $this->StorageProvider()->Get(null,
			\X2Mail\Engine\Providers\Storage\Enumerations\StorageType::NOBODY, 'Cache/Timers', '');
...
//			$oSettingsLocal->SetConf('Theme', $this->ValidateTheme($oConfig->Get('webmail', 'theme', 'Default')));
		...
		$this->setSettingsFromParams($oSettings, 'Layout', 'int', function ($iValue) {
			return (int) (\in_array((int) $iValue, array(\X2Mail\Engine\Enumerations\Layout::NO_PREVIEW,
				\X2Mail\Engine\Enumerations\Layout::SIDE_PREVIEW, \X2Mail\Engine\Enumerations\Layout::BOTTOM_PREVIEW)) ?
					$iValue : \X2Mail\Engine\Enumerations\Layout::SIDE_PREVIEW);
		});
